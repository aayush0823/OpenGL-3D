...
Ball::Ball(float x, float y,float z, color_t color) {
    ...
    for(int i=0;i<9*50;i+=9)
    {
    	vertex_buffer_data3[i]=0.2*cos(2*pi/(float)50*(float)t);
        vertex_buffer_data3[i+1]=0.2*sin(2*pi/(float)50*(float)t);
        vertex_buffer_data3[i+2]=0.0f;
        vertex_buffer_data3[i+3]=0.2*cos(2*pi/(float)50*(float)(t+1));
        vertex_buffer_data3[i+4]=0.2*sin(2*pi/(float)50*(float)(t+1));
    ...
    this->object3 = create3DObject(GL_TRIANGLES, 3*50, vertex_buffer_data3, COLOR_ORANGE, GL_FILL);
}
